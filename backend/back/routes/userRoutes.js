...
router.post("/register", async (req,res)  => {
    try {
        ...
        const newUser = { userName, email, password:hashedPassword, id: Date.now().toString(),role:"admin"}  // לזכור לשנות בעתיד ליוזר רול חשוב!  
        users.push(newUser)
        res.status(201).json({ message: "וולידציה עברה בהצלחה! משתמש נרשם." ,userId: newUser.id, newUser});
    } catch (error) {
...
router.post("/login" ,async(req,res) => {
    ...
    if (!email || !password) {
        ...
        if (isMatch) {
            const token = jwt.sign({email: findUser.email, id:findUser.id, role:findUser.role},process.env.SECRET , { expiresIn: '1h' });
            return res.status(200).json({ message: "  ההתחברות בוצעה בהצלחה",token });
